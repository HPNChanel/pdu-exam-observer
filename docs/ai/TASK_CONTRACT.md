# Task contract v23 — approved full-system implementation

Updated: 2026-09-08. Active scope: COMPLETION_2026_09_08.md.

Technical delivery status: LOCAL_TECHNICAL_DELIVERY_VERIFIED. Final candidate
and Colab artifacts, source/manifest bindings, test results and external
limitations are recorded in the completion ledger. The external research and
device conditions below remain unchanged.

USER_STATED: complete M2–M7 technical implementation, the Vietnamese workspace,
shared preprocessing, independent rules, review/export/withdrawal, Colab code,
model import/inference/evaluation and local packaging. Current-camera checks
without retained participant media are authorized. Real data deletion is only
rehearsed with tool-created fixtures; production use still needs native authority.

Acceptance requires current source tests, actual UI evidence, synthetic model
round trip, source-bound local ZIP and same-host relocated execution. A new
machine test is explicitly deferred, recorded UNVERIFIED, and does not block
technical delivery. No academic/research-performance approval is inferred.

Preserve the canonical proposal and existing unrelated work. Do not commit,
push, deploy, publish, release or collect participants. Historical contracts
below remain an audit record of narrower milestones, not the current scope.

## Historical task contract v22
# Task Contract v22

## M2-S3E-A same-host isolated portability boundary

`USER_STATED`: the user approved the S3E-A plan and authorized the bounded
diagnostic, corrected same-host acceptance execution, and local verification.
...
The contract permits deterministic handoff construction, exactly two
same-host relocation profiles, Windows PowerShell 5.1 execution, loopback-only
synthetic preflight/nominal/export/reproduction, canonical evidence,
governance reconciliation, and verification. It does not permit claiming a
clean-machine result, camera/device access, participant contact, signing,
distribution, deployment, release, real deletion, or M3.
...
## Historical M2-S3D task contract

## M2-S3D packaged evidence round-trip boundary

`USER_STATED`: the user approved the S3D implementation plan and authorized a
single local lifecycle commit plus push to `main` after every gate passes.

Current status:
`M2_S3D_PACKAGED_SYNTHETIC_EVIDENCE_ROUND_TRIP_LOCALLY_VERIFIED_SAME_HOST_ONLY_DEVICE_UNVERIFIED_NO_RELEASE_AUTHORITY`.

The contract permits a new ignored S3D candidate lineage, two deterministic
builds, two fixed packaged round-trip cycles, monitor-only S2D export, closed
stdin-only S2E reproduction, canonical receipts, governance reconciliation,
verification, commit, and push. It does not permit an HTTP reproduction API,
arbitrary path/URL/device input, camera access, participant contact, signing,
promotion, distribution, deployment, release, real deletion, or M3.
...
## Historical M2-S3C task contract

## M2-S3C packaged synthetic runtime smoke boundary

`USER_STATED`: the user authorized the next task, local lifecycle commit, and
push after task completion. The runtime scope remained the fixed local
...
The contract permits two fixed candidate process invocations, loopback-only
health/authentication, built-in synthetic preflight and nominal runs, forced
process-tree shutdown, owned-temp cleanup, canonical evidence, RP2
reconciliation, tests, commit, and push. It does not permit camera/device
enumeration, participant contact, evidence export, signing, promotion,
distribution, deployment, release, real deletion, or M3.
