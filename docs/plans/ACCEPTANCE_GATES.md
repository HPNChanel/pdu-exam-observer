...
## G24 - M2-S3E-A same-host isolated portability acceptance
...
G24 requires two relocation profiles, six packaged candidate processes, two
preflight runs, two nominal runs, four S2D exports, four exact S2E
reproductions, byte-identical projections, loopback-only observation,
candidate immutability, and complete cleanup. It does not establish a result
from a separate clean machine or VM and grants no physical-device,
participant, distribution, signing, deployment, or release authority.
