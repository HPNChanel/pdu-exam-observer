"""Run the built-in M2-S2B simulated nominal flow in an owned temporary root."""
...
from pdu_exam_observer.m2_synthetic_nominal_fixture import (  # noqa: E402
    BUILTIN_RUN_ID,
    build_builtin_nominal_bundle,
)

_ENCODER_POLICY_PREIMAGE = b"M2-S2B-SIMULATED-ENCODER-NOT-INVOKED-V1"
...
def _source_digest() -> str:
    names = (
        ...
        "m2_synthetic_nominal_fixture.py",
    )
    bindings = {
...
def _rejected() -> dict[str, object]:
    return {
        ...
        "schema_version": 1,
        "status": "M2_S2B_SYNTHETIC_NOMINAL_20M_NOT_PERSISTED",
...
def _run() -> tuple[dict[str, object], int]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="pdu-m2-s2b-") as temporary:
            ...
            study = backend.create_study("m2-s2b", idempotency_key="study-m2-s2b")
            participant = backend.create_participant(
                str(study["study_id"]), idempotency_key="person-m2-s2b"
            )
            session = backend.create_research_session(
                str(study["study_id"]),
                str(participant["participant_id"]),
                idempotency_key="session-m2-s2b",
            ...
            bundle = build_builtin_nominal_bundle(pose_digest)
            ...
            receipt = core.execute(
                SyntheticNominalRequest(
                    ...
                    intent_id="intent-m2-s2b-nominal",
                    artifact_id="artifact-m2-s2b-nominal",
        ...
        success = (
            ...
            and result.get("run_kind") == "NOMINAL_20M"
            and removed
        )
